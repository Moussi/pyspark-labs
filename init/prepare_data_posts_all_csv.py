...
import datetime
import csv
from labs.helpers.spark import Session
...
class Utils:
    ...
    def init():
        ...
        xml_posts = spark.sparkContext.newAPIHadoopFile(
            "file:///home/moussi/Desktop/Projects/pyspark-cloudera-labs/dataset/Posts.xml",
            'com.databricks.spark.xml.XmlInputFormat', 'org.apache.hadoop.io.Text', 'org.apache.hadoop.io.Text',
            conf={'xmlinput.start': '<row', 'xmlinput.end': '/>'})
        each_post = xml_posts.map(lambda x: x[1])
        post_fields = each_post.map(Utils.processXmlFields)
        ...
        postDF.write.format('com.databricks.spark.csv').options(header='false').save(
            '/home/moussi/Desktop/Projects/pyspark-cloudera-labs/pyspark-lab/datasets/cloudera/stackexchange2/posts_all_csv')


if __name__ == "__main__":
    os.environ["ENV_CURRENT"] = "local"
    Utils.init()
